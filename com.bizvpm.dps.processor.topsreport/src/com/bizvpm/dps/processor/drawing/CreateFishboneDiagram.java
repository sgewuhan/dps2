package com.bizvpm.dps.processor.drawing;
...
public class CreateFishboneDiagram implements IProcessorRunable {
	...
	public ProcessResult run(ProcessTask processTask, IProgressMonitor monitor, IProcessContext context)
		...
		graphics2D.drawImage(bufferedImage.getScaledInstance(bufferedImage.getWidth(null), bufferedImage.getHeight(null), Image.SCALE_SMOOTH), 0, 0, null);
        ...
        graphics2D.drawPolygon(xPoints1, yPoints1, 3);
        
        //设置文字内容、位置  
        graphics2D.drawString(name,width-(100+(name.length()*10)-50),height/2+5);  
        ...
        for (String key : map.keySet()) {
        	...
        	newLine(parameter,key,map.get(key),graphics2D);
			i++;	
		} 
	 ...
	 * @param list
	 */
	private void getPrameter(int x1,int y1,int x2,int y2,int len,Graphics2D g2,Map<String,Object> map){
		int i =1;
	...
	private void newLine(Map<String, Integer> parameter,String value,Object data,Graphics2D g2){
		...
		if(isRecursion(data)){
    		getPrameter(parameter.get(DRAW_LINE_X1),parameter.get(DRAW_LINE_Y1),parameter.get(DRAW_LINE_X2),parameter.get(DRAW_LINE_Y2),parameter.get(DRAW_LINE_LEN),g2,(Map<String,Object>)data);
    	}
	}
