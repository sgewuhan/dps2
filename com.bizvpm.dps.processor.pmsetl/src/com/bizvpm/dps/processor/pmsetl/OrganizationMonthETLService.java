...
public class OrganizationMonthETLService extends AbstractMonthETLService {
	...
	private void createOrganizationMonthData(MongoCollection<Document> prjColl, MongoCollection<Document> oMDCol,
		...
		// 获取组织月绩效数据
		List<? extends Bson> pipeline = Arrays.asList(new Document("$graphLookup", new Document()
				.append("from", "organization").append("startWith", "$_id").append("connectFromField", "_id")
				.append("connectToField", "parent_id").append("as", "children").append("depthField", "level")),
				new Document("$addFields", new Document("children", new Document("$concatArrays",
						Arrays.asList("$children._id", Arrays.asList("$_id"))))),
				new Document("$lookup", new Document("from", "project")
						.append("let", new Document("impUnit_id", "$children")).append("pipeline", Arrays.asList(
								new Document("$project", new Document("_id", true).append("impUnit_id", true)),
								new Document("$match", new Document("$expr", new Document("$and",
										Arrays.asList(
												new Document("$in", Arrays.asList("$impUnit_id", "$$impUnit_id"))))))))
						.append("as", "project_id")),
				new Document("$addFields", new Document("project_id", "$project_id._id")), new Document("$addFields",
						new Document("discountRate", discountRate).append("nowYear", year).append("sDate", year
								+ month)),
				new Document("$lookup", new Document("from", "salesforecast").append("let",
						new Document("project_id", "$project_id").append("discountRate", "$discountRate")
								.append("nowYear", "$nowYear"))
						.append("pipeline", Arrays.asList(
								new Document("$match",
										new Document("$expr", new Document("$and", Arrays.asList(
												new Document("$in", Arrays.asList("$project_id", "$$project_id")))))),
								...
								new Document("$addFields",
										new Document("exponent", new Document("$subtract", Arrays.asList(
												new Document("$toInt", "$_id"), new Document("$toInt", "$$nowYear"))))),
								new Document("$addFields", new Document()
										.append("presentRevenue", new Document("$cond", Arrays
												.asList(new Document("$gte", Arrays.asList("$exponent", 0.0)),
														new Document("$divide",
				...
				new Document().append("$lookup", new Document("from", "salesMonthData")
						.append("let",
								new Document("project_id", "$project_id").append("discountRate", "$discountRate")
						...
						.append("pipeline",
								Arrays.asList(
										new Document("$match", new Document("$expr",
												new Document("$and",
														Arrays.asList(new Document("$in", Arrays.asList("$project_id",
																"$$project_id")), new Document(
																		"$lte",
																		Arrays.asList(
																				new Document("$concat",
																						Arrays.asList("$GJAHR",
				...
				new Document("$lookup",
						new Document("from", "cbsSubject")
								.append("let",
										new Document("project_id", "$project_id").append("discountRate",
												"$discountRate").append("nowYear",
														"$nowYear"))
								.append("pipeline", Arrays.asList(new Document("$lookup", new Document("from", "cbs")
										.append("localField", "cbsItem_id").append("foreignField", "_id").append("as",
												"cbs")),
										...
										new Document("$unwind",
												new Document("path", "$project").append("preserveNullAndEmptyArrays",
														true)),
										new Document("$addFields", new Document("project_id", new Document("$ifNull",
				...
				new Document("$unwind", new Document("path",
						"$salesRealityTotalData").append("preserveNullAndEmptyArrays",
								true)),
				new Document("$lookup",
						new Document("from", "cbsSubject")
								.append("let",
				...
				new Document("$lookup", new Document()
						.append("from", "salesMonthData").append("let",
								new Document("project_id", "$project_id").append("discountRate", "$discountRate")
										.append("nowYear", "$nowYear").append("sDate", "$sDate"))
						.append("pipeline",
								Arrays.asList(
				...
				new Document("$unwind", new Document("path",
						"$cbsMonthData").append("preserveNullAndEmptyArrays",
								true)),
				new Document("$project",
						new Document("impUnit_id", "$_id")
								...
								.append("monthBudget",
										new Document("$ifNull", Arrays.asList("$cbsMonthData.budget", 0.0)))
				...
				new Document("$addFields", new Document()
						.append("forecastPP", new Document("$cond", Arrays.asList(
								new Document("$eq", Arrays.asList("$forecastYearAvgProfit", 0.0)), 0.0,
								new Document("$multiply",
										Arrays.asList(new Document("$divide",
												Arrays.asList("$budget", "$forecastYearAvgProfit")), 12.0)))))
						.append("forecastPresentPP",
								new Document("$cond",
										Arrays.asList(
												new Document(
														"$eq", Arrays.asList("$forecastYearPresentAvgProfit", 0.0)),
												0.0,
												new Document("$multiply",
														Arrays.asList(
																new Document("$divide",
						...
						.append("realityPP", new Document("$cond", Arrays.asList(
								new Document("$eq", Arrays.asList("$realityYearAvgProfit", 0.0)), 0.0,
								new Document("$multiply",
										Arrays.asList(new Document("$divide",
												Arrays.asList("$cost", "$realityYearAvgProfit")), 12.0)))))
						.append("realityPresentPP",
								new Document("$cond",
										Arrays.asList(
												new Document("$eq", Arrays.asList("$realityYearPresentAvgProfit", 0.0)),
												0.0,
												new Document("$multiply",
														Arrays.asList(
																new Document("$divide",
