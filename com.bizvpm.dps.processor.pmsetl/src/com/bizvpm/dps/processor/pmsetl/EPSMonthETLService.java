	...
	private void createEPSMonthData(MongoCollection<Document> prjColl, MongoCollection<Document> eMDCol,
		...
		List<Bson> pipeline = Arrays.asList(
				new Document("$graphLookup",
						new Document("from", "eps").append("startWith", "$_id").append("connectFromField", "_id")
								.append("connectToField", "parent_id").append("as", "children")
								.append("depthField", "level")),
				new Document("$addFields",
						new Document("children",
								new Document("$concatArrays", Arrays.asList("$children._id", Arrays.asList("$_id"))))),
				new Document("$lookup",
						new Document("from", "project").append("let", new Document("eps_id", "$children"))
								.append("pipeline", Arrays.asList(
										new Document("$match",
												new Document("$expr", new Document("$and", Arrays.asList(
														new Document("$in", Arrays.asList("$eps_id", "$$eps_id")))))),
										new Document("$addFields",
												new Document("discountRate", discountRate).append("nowYear", year)
										...
										new Document("$lookup", new Document("from", "cbsSubject").append("let",
												new Document("project_id", "$_id").append("discountRate",
										...
										new Document("$lookup", new Document("from", "salesforecast")
												...
												.append("pipeline", Arrays.asList(
														new Document("$match", new Document(
																"$expr", new Document("$and", Arrays.asList(
				...
				new Document("$addFields", new Document("pj.eps_id", "$_id")
						.append("pj.forecastPP", new Document("$cond",
								Arrays.asList(new Document("$eq", Arrays.asList("$pj.forecastYearAvgProfit", 0.0)), 0.0,
										new Document("$multiply",
												Arrays.asList(new Document("$divide",
														Arrays.asList("$pj.budget", "$pj.forecastYearAvgProfit")),
														12.0)))))
						.append("pj.forecastPresentPP",
								new Document("$cond",
										Arrays.asList(
												new Document(
														"$eq", Arrays.asList("$pj.forecastYearPresentAvgProfit", 0.0)),
												0.0,
												new Document("$multiply",
														Arrays.asList(
																new Document("$divide",
																...
																12.0)))))
						.append("pj.realityPP", new Document("$cond",
								Arrays.asList(new Document("$eq", Arrays.asList("$pj.realityYearAvgProfit", 0.0)), 0.0,
										new Document("$multiply",
												Arrays.asList(
														new Document("$divide",
																Arrays.asList("$pj.cost", "$pj.realityYearAvgProfit")),
														12.0)))))
						.append("pj.realityPresentPP",
								new Document("$cond",
										Arrays.asList(
												new Document(
														"$eq", Arrays.asList("$pj.realityYearPresentAvgProfit", 0.0)),
												0.0,
												new Document("$multiply",
														Arrays.asList(
																new Document("$divide",
						...
						.append("pj.realityPresentROI", new Document("$cond",
								Arrays.asList(new Document("$eq", Arrays.asList("$pj.presentCost", 0.0)), 0.0,
										new Document("$divide",
												Arrays.asList("$pj.realityYearPresentAvgProfit", "$pj.presentCost")))))
						.append("pj.forecastNPV", new Document("$add",
								Arrays.asList(new Document("$multiply", Arrays.asList(-1.0, "$pj.presentBudget")),
										"$pj.forecastPresentProfit")))
						.append("pj.realityNPV",
								new Document("$add",
										Arrays.asList(new Document("$multiply", Arrays.asList(-1.0, "$pj.presentCost")),
												"$pj.realityPresentProfit")))
						.append("pj.forecastNPVR",
								new Document("$cond",
										Arrays.asList(new Document("$eq", Arrays.asList("$pj.presentBudget", 0.0)), 0.0,
												new Document("$divide", Arrays.asList(
														new Document("$add",
