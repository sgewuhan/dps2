...
public class StopProcessor extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection != null && !selection.isEmpty()) {
			try {
				...
				Activator.getDefault().getProcessorManager().stopProcessor(config);
				IWorkbenchPart part = HandlerUtil.getActivePart(event);
				if (part instanceof IRefreshable) {
					((IRefreshable) part).refresh(config);
				}
			} catch (Exception e) {
				MessageDialog.open(MessageDialog.ERROR, HandlerUtil.getActiveShell(event), "´¦ÀíÆ÷Í£Ö¹Ê§°Ü", e.getMessage(),
						SWT.SHEET);
