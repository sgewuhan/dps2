...
public class QueryProperties
{
    ...
    ReadPreference getTaggableReadPreference()
    {
		...
		List<TagSet> tagSet = new ArrayList<TagSet>();

        appendTagSets(firstTagSet,tagSet);
        
        DBObject[] remainingTagSets = getSecondaryObjectSets( tagSets );        
        if(remainingTagSets!=null){
        	for (int i = 0; i < remainingTagSets.length; i++) {
                appendTagSets(remainingTagSets[i],tagSet);
			}
        }
        try
        {
           ReadPreference.valueOf( readPref.getName(), tagSet );
        }
	...
	private List<TagSet> appendTagSets(DBObject firstTagSet, List<TagSet> tagSet) {
        List<Tag> tags = new ArrayList<Tag>();
        Iterator<String> iter = firstTagSet.keySet().iterator();
        while(iter.hasNext()){
        	String key = iter.next();
        	tags.add(new Tag(key,(String) firstTagSet.get(key)));
        }
