...
public class MDbOperation {
	...
	protected void prepare(DBCollection dbCollection) throws OdaException {
		...
		try {
			...
			DriverUtil.getLogger().log(Level.SEVERE,
					"Encountered RuntimeException in QueryModel#prepareQuery(DBCollection).", ex); //$NON-NLS-1$
	...
	static void applyPropertiesToCursor(DBCursor rowsCursor, QueryProperties queryProps, boolean includeSortExpr) {
		if (includeSortExpr) // normally done only when executing a query to get
								// full result set
		{
	...
	static Iterable<DBObject> callAggregateCmd(DBCollection dbCollection, QueryProperties queryProps)
			throws OdaException {
		if (!queryProps.hasAggregateCommand())
			return null;
		...
		try {
			output = addlOps != null ? dbCollection.aggregate(firstOp, addlOps) : dbCollection.aggregate(firstOp);
	...
	static MapReduceOutput callMapReduceCmd(DBCollection dbCollection, QueryProperties queryProps) throws OdaException {
		...
		// check if mapreduce key is already specified in user-defined
		// expression
		DBObject _mapReduceCmd;
		if (command.containsField(QueryModel.MAP_REDUCE_CMD_KEY)
				|| command.containsField(QueryModel.MAP_REDUCE_CMD_KEY2))
			_mapReduceCmd = command;
		else {
			// add MapReduce input collection as first entry in command Map
			_mapReduceCmd = new BasicDBObject(QueryModel.MAP_REDUCE_CMD_KEY, dbCollection.getName());
			_mapReduceCmd.putAll(command); // copy existing command entries
		}
		...
		try {
			MapReduceCommand mapReduceCmd = new MapReduceCommand(dbCollection, null, null, null, null, _mapReduceCmd);
