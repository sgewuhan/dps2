...
public class WPSProcessor implements IProcessorRunable {
	...
	private void convert() throws Exception {
		...
		AbstractWPSConverter wpsConverter = AbstractWPSConverter.getInstance(sourceType);
		try {
			ComThread.InitSTA();
			app = wpsConverter.getActiveXComponent();
			dis = wpsConverter.openDocument(app, filename);
			wpsConverter.convert(dis, toFilename);
		} catch (Exception e) {
			throw e;
		} finally {
			wpsConverter.dispose(app, dis);
			ComThread.Release();
		}
	}
